...
export default function EditTrafficSignDialog({ sign, categories }: EditTrafficSignDialogProps) {
    ...
    const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
        if (e.target.files && e.target.files[0]) {
            ...
            try {
                ...
                toast({ title: "تم رفع الصورة", description: "الصورة الجديدة جاهزة. احفظ التغييرات لتطبيقها." });
            } catch (error) {
                console.error("Image upload failed: ", error);
                toast({ title: "فشل الرفع", description: "تعذر رفع الصورة الجديدة.", variant: "destructive" });
            } finally {
    ...
    const onSubmit = async (values: z.infer<typeof formSchema>) => {
        ...
        try {
            ...
            toast({ title: "خطأ", description: "فشل تحديث إشارة المرور.", variant: "destructive" });
        } finally {
    ...
    return (
        <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent className="sm:max-w-[480px]">
                <DialogHeader>
                    <DialogTitle>تعديل إشارة المرور</DialogTitle>
                    <DialogDescription>
                        تحديث تفاصيل `{sign.name}`.
                    </DialogDescription>
                </DialogHeader>
                <Form {...form}>
